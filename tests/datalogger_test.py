...
def test_log_group_modify(server):
	assert datalogger.log_group.modify_log_group(server, {"datalogger.LOG_GROUP_USE_LOCAL_TIME_FOR_TIMESTAMP_INSERTS": False}, log_group= log_group_data1['common.ALLTYPES_NAME'], force=True)
	
	assert datalogger.log_group.modify_log_group(server,{"datalogger.LOG_GROUP_USE_LOCAL_TIME_FOR_TIMESTAMP_INSERTS": True},log_group= log_group_data1['common.ALLTYPES_NAME'])
...
def test_log_item_modify(server):
	assert datalogger.log_items.modify_log_item(server, log_group_name, {"datalogger.LOG_ITEM_NUMERIC_ID": "1"} , log_item= log_item1['common.ALLTYPES_NAME'], force=True)

	assert datalogger.log_items.modify_log_item(server, log_group_name, {"datalogger.LOG_ITEM_NUMERIC_ID": "0"} ,log_item= log_item1['common.ALLTYPES_NAME'])
...
def test_log_item_get(server):
	...
	assert datalogger.log_group.modify_log_group(server, {"datalogger.LOG_GROUP_TABLE_FORMAT": 1}, log_group= log_group_name, force=True)
...
def test_mapping_modify(server):
	...
	assert datalogger.mapping.modify_mapping(server, log_group_name, {"datalogger.TABLE_ALIAS_SQL_LENGTH_QUALITY": 10} ,mapping= mapping_list[0]['common.ALLTYPES_NAME'], force=True)

	assert datalogger.mapping.modify_mapping(server, log_group_name, {"datalogger.TABLE_ALIAS_SQL_LENGTH_QUALITY": 15} ,mapping= mapping_list[0]['common.ALLTYPES_NAME'])
...
def test_trigger_modify(server):
	assert datalogger.triggers.modify_trigger(server, log_group_name, {"datalogger.TRIGGER_STATIC_INTERVAL": 1000} ,trigger= trigger1['common.ALLTYPES_NAME'], force=True)

	assert datalogger.triggers.modify_trigger(server, log_group_name, {"datalogger.TRIGGER_STATIC_INTERVAL": 500} ,trigger= trigger1['common.ALLTYPES_NAME'])
